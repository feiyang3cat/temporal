...
// (a) skip-to-timer1 (DisabledAfterFastForward=false), (b) fast-forward-disable.
func (s *TimeSkippingFastForwardFunctionalSuite) TestFastForward_PauseLifecycle() {
	...
	second := transitions[1].GetWorkflowExecutionTimeSkippingTransitionedEventAttributes()
	s.True(second.GetDisabledAfterFastForward(), "second transition must be the fastForward-disable event")
...
func (s *TimeSkippingFastForwardFunctionalSuite) TestFastForward_RunTimeoutBeforeFastForward() {
	...
	s.False(transitions[0].GetWorkflowExecutionTimeSkippingTransitionedEventAttributes().GetDisabledAfterFastForward(),
		"run timeout caps the skip before fastForward is reached: DisabledAfterFastForward must be false")
