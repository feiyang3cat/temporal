...
type TimeSkippingFastForwardNotifier struct {
	lock        sync.Mutex
	subscribers map[chasm.ExecutionKey]map[string]chan *commonpb.TimeSkippingInfo
}

func NewTimeSkippingFastForwardNotifier() *TimeSkippingFastForwardNotifier {
	return &TimeSkippingFastForwardNotifier{
		subscribers: make(map[chasm.ExecutionKey]map[string]chan *commonpb.TimeSkippingInfo),
	}
}
...
func (n *TimeSkippingFastForwardNotifier) Subscribe(
	...
	if !ok {
		byID = make(map[string]chan *commonpb.TimeSkippingInfo)
