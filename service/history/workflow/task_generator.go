...
// on each TimeSkippingDetail. Mirrors TimerInfo.TaskStatus's idempotency role for user timers
...
// Idempotent via TimeSkippingInfo.TimeSkippingDetails: only emits when at least one detail's
...
func (r *TaskGeneratorImpl) RegenerateTimerTasksForTimeSkipping() error {
	...
	hasUnmarked := false
	for _, d := range tsi.GetTimeSkippingDetails() {
