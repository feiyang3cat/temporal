...
// merge contract: TimeSkippingDetail.TaskRegenStatus is per-cluster local
...
func TestApplyIncomingTimeSkippingInfo_PreservesLocalRegenStatus(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name:    "current nil resets all incoming to None",
			current: nil,
			incoming: &persistencespb.TimeSkippingInfo{
				TimeSkippingDetails: []*persistencespb.TimeSkippingDetail{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Len(t, current.TimeSkippingInfo.TimeSkippingDetails, len(tc.wantStatuses))
			for _, d := range current.TimeSkippingInfo.TimeSkippingDetails {
...
func TestApplyIncomingTimeSkippingInfo_ClonesToBreakAliasing(t *testing.T) {
	...
	require.NotSame(t, current.TimeSkippingInfo.TimeSkippingDetails[0],
		incoming.TimeSkippingInfo.TimeSkippingDetails[0],
		"detail must also be cloned")

	// Mutate local; incoming must not change.
	current.TimeSkippingInfo.TimeSkippingDetails[0].TaskRegenStatus = TimeSkippingTaskRegenStatusRegenerated
	require.Equal(t, TimeSkippingTaskRegenStatusRegenerated,
		incoming.TimeSkippingInfo.TimeSkippingDetails[0].GetTaskRegenStatus(),
