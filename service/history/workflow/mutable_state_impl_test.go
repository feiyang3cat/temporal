...
func (s *mutableStateSuite) TestAddStartChildWorkflowExecutionInitiatedEvent_TimeSkippingSnapshot() {
	cases := []struct {
		...
		{
			name: "FastForward is never propagated to children → cleared from child snapshot",
			parentTSI: &persistencespb.TimeSkippingInfo{
		...
		{
			name: "DisableChildPropagation=true → no config propagated, virtual time still propagated",
			parentTSI: &persistencespb.TimeSkippingInfo{
				Config: &workflowpb.TimeSkippingConfig{
					Enabled:                 true,
		...
		{
			name: "DisableChildPropagation=false → child inherits Enabled, FastForward still cleared",
			parentTSI: &persistencespb.TimeSkippingInfo{
				Config: &workflowpb.TimeSkippingConfig{
					Enabled:                 true,
					FastForward:             durationpb.New(2 * time.Hour),
...
func (s *mutableStateSuite) TestApplyWorkflowExecutionTimeSkippingTransitionedEvent() {
	...
	makeEvent := func(eventTime time.Time, targetTime *time.Time, disabledAfterFastForward bool) *historypb.HistoryEvent {
