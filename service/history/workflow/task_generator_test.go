...
func TestTaskGeneratorImpl_RegenerateTimerTasksForTimeSkipping_FastForwardTimer(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			mutableState.EXPECT().GetWorkflowKey().Return(tests.WorkflowKey).AnyTimes()
			// GetCurrentVersion is consulted when the fast-forward task is regenerated so its
			// Version is the current failover version for CheckTaskVersion.
			...
			require.Equal(t, currentVersion, bt.Version)
...
// requireAllFieldsPopulated fails if any field of the task struct is left at its zero
// value, except the names in ignore. It recurses into embedded (anonymous) structs such
// as definition.WorkflowKey so each of NamespaceID/WorkflowID/RunID is checked
// individually. Named struct fields (e.g. time.Time) are treated as leaves via IsZero.
//
// Caveat: this guard is only as good as its inputs. A field reads as "populated" merely
// by being non-zero — it does NOT prove regeneration set it to the *correct* value (those
// per-field value assertions live in the sibling _ExecutionTimers/_FastForwardTimer/
// _BackoffTimer tests). It can also be defeated if the test setup happens to leave a
// field at its zero value for unrelated reasons. Its one job is to catch field-omission
// drift: a field added to a task struct and wired into the canonical generator but
// forgotten in RegenerateTimerTasksForTimeSkipping. When that happens the new field is
// neither populated nor in the ignore-list, so this fails and forces an explicit decision.
...
//
// Unlike activity-retry regen (which reuses the shared GenerateActivityRetryTasks), the
// user-timer, execution/run-timeout, backoff, and fast-forward blocks each construct their
// task inline because the canonical generators bundle gating and side effects that regen
// must bypass (see the comment on RegenerateTimerTasksForTimeSkipping). That inline
// construction can silently drift from a task struct's field set. This test drives a single
// mutable state that emits every hand-built task type at once and asserts each one has all
// of its fields populated (minus an explicit ignore-list), so a newly added-and-forgotten
...
func TestTaskGeneratorImpl_RegenerateTimerTasksForTimeSkipping_AllFieldsPopulated(t *testing.T) {
	...
	// current (failover) version. Distinct values mirror production sourcing.
