...
func TestTaskGeneratorImpl_RegenerateTimerTasksForTimeSkipping(t *testing.T) {
	...
	mutableState.EXPECT().GetExecutionInfo().Return(&persistencespb.WorkflowExecutionInfo{
		TimeSkippingInfo: &persistencespb.TimeSkippingInfo{
			...
			TimeSkippingDetails: []*persistencespb.TimeSkippingDetail{{TaskRegenStatus: TimeSkippingTaskRegenStatusNone}},
		},
	}).AnyTimes()
...
// asserts that a successful regen flips every unmarked TimeSkippingDetail to
...
func TestTaskGeneratorImpl_RegenerateTimerTasksForTimeSkipping_MarksDetailsAndIsIdempotent(t *testing.T) {
	...
	require.NoError(t, taskGenerator.RegenerateTimerTasksForTimeSkipping())
	require.Greater(t, emitCount, 0, "first call must emit at least one task")
	for _, d := range tsi.TimeSkippingDetails {
	...
	require.NoError(t, taskGenerator.RegenerateTimerTasksForTimeSkipping())
	require.Equal(t, emitsAfterFirst, emitCount, "second call must be a no-op")
}
